...
def test_plain_reporter_emits_exact_columns_cadence_and_dfl_note() -> None:
    stream = StringIO()
    reporter = YoloProgressReporter(
        every_n_steps=2, reg_max=0, stream=stream, use_tqdm=False
    )
    ...
    assert lines[1] == "detection_dfl=0 (reg_max=0: direct box regression, DFL disabled)"
    assert len(lines) == 4  # header, note, cadence row, forced last row
    fields = lines[2].split("\t")
    assert len(fields) == len(TRAIN_PROGRESS_COLUMNS)
    assert fields[0] == "1/3"
    assert fields[1] == "2.00G"
    assert fields[TRAIN_PROGRESS_COLUMNS.index("detection_dfl")] == "0.0000"
    assert fields[TRAIN_PROGRESS_COLUMNS.index("Instances")] == "7"
